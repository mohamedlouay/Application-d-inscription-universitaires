...
public class ExpansionHeader extends FrameLayout {
    ...
    public Boolean getNoChoix() {
        return noChoix;
    }

    public void setNoChoix(Boolean noChoix) {
        this.noChoix = noChoix;
    }

    Boolean noChoix = false;

    public void setWithoutCheck(Boolean withoutCheck) {
        this.withoutCheck = withoutCheck;
    }

    public Boolean getWithoutCheck() {
        return withoutCheck;
    }

    Boolean withoutCheck = false;
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray a = context.obtainStyledAttributes(attrs, com.example.plpla.R.styleable.ExpansionHeader);
            if (a != null) {
                setHeaderRotationExpanded(a.getInt(com.example.plpla.R.styleable.ExpansionHeader_expansion_headerIndicatorRotationExpanded, headerRotationExpanded));
                setHeaderRotationCollapsed(a.getInt(com.example.plpla.R.styleable.ExpansionHeader_expansion_headerIndicatorRotationCollapsed, headerRotationCollapsed));
                setHeaderIndicatorId(a.getResourceId(com.example.plpla.R.styleable.ExpansionHeader_expansion_headerIndicator, headerIndicatorId));
                setExpansionLayoutId(a.getResourceId(com.example.plpla.R.styleable.ExpansionHeader_expansion_layout, expansionLayoutId));
                setToggleOnClick(a.getBoolean(com.example.plpla.R.styleable.ExpansionHeader_expansion_toggleOnClick, toggleOnClick));
                a.recycle();
            }
        }
    }
            ...
            setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    if (toggleOnClick) {
                        ...
                        if (!withoutCheck) checkboxHeader.setChecked(true);
                    }
                    if (!expansionLayout.isExpanded()) checkboxHeader.setChecked(false);
                }
            });
